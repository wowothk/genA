...
params_c = [i/10 for i in range(10) if i!=0]
params_m = [i/10 for i in range(10) if i!=0]
run = 10

running_time=0
while running_time < run:
    print(running_time)
    for pc in params_c:
        for pm in params_m:
            ...
            while generation < the_number_of_generation:    
                ...
                #    ### mutation
                random_mutation = [random.random() for i in range(len(lambdas))]
                index_parent = parentMutation(random_mutation, pm)
            ...
            df200 = pd.DataFrame({
                                "generasi":the_number_of_generation,
                                "pc":pc,
                                "pm":pm,
...
export_excel = df.to_excel (r'/home/rudi/Documents/skripsi/perbandinganparameter2000.xlsx', index = None, header=True)
